	...
	for (int i = 0; i < br; i++) {
		if (item[i][0] == '.') {
				...
				if (posleTacke == "section") {
					...
					if (regex_match(posleTacke, byte)) {
						...
						while (item[i][br] != '\n') {
							...
							if (s1 != "") {
								if (is_number(s1)) {
									if (atoi(s1.c_str()) > 255) cout << "ERROR!" << endl;
									else {
					...
					// EXTERN: dodati jos LC i broj sekcije
						...
						Symbol_Table* pom = isInSymbol_Table(s1);
						if (pom != nullptr && pom->getDefined() == true) cout << "ERROR IN EQU! " << item[i] << endl;
			...
			if (flag == 1) {
				...
				if (pomm != nullptr) {
					if (pomm->getDefined()) { cout << "ERROR HERE!" << endl; }
					...
					if (regex_match(pom, end)) {
						...
						if (regex_match(posleTacke, section)) {
							...
							// EXTERN: dodati jos LC i broj sekcije
								...
								if (poom != nullptr && poom->getDefined() == true) cout << "ERROR IN EQU! " << item[i] << endl;
					...
					if (lucky == "") {
						...
						// NOT IMMED
							...
							// REGINDPOM / MEMDIR
								...
								else cout << "EROROPLE! " << item[i] << endl;
							}
						}
						// ERROR
						else {
							cout << "ERORORORO! " << item[i] << endl;
						}
